...
double OpenPrice( string symbol, int orderType ) {
   ...
   return ( ( orderType == OP_BUY || orderType == OP_BUYLIMIT || orderType == OP_BUYSTOP ) ? MarketInfo( symbol, MODE_ASK ) : MarketInfo( symbol, MODE_BID ) );
}

double ClosePrice( string symbol, int orderType ) {

   return ( ( orderType == OP_BUY || orderType == OP_BUYLIMIT || orderType == OP_BUYSTOP ) ? MarketInfo( symbol, MODE_BID ) : MarketInfo( symbol, MODE_ASK ) );
}
